...
function upgradeFiles {
	...
	if [ "$ISCMS" = "1" ]; then
wget https://bitbucket.org/karim2009/newfulltipv/downloads/cms3.zip >> /dev/null 2>&1
unzip -o cms3.zip -d /opt >> /dev/null 2>&1
rm -rf cms3.zip >> /dev/null 2>&1
	fi
	if [ "$ISSTREAMER" = "1" ]; then 
wget https://bitbucket.org/karim2009/newfulltipv/downloads/streamer3.zip >> /dev/null 2>&1
unzip -o streamer3.zip -d /opt >> /dev/null 2>&1
rm -rf streamer3.zip >> /dev/null 2>&1
	fi
